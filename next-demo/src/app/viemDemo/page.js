...
export default function ViemDemo() {
  ...
  const RPC_URL = "https://sepolia.infura.io/v3/12cde4ece77f415ba071bb43fba83f40"
...
const getBalances = async () => {
  ...
  try {
    setStatus('查询 ETH 和 Token 余额...');
      console.log('console.log(process.env)console.log(process.env)console.log(process.env)',process.env)
    const account = privateKeyToAccount(`0x${privateKey}`);
    const publicClient = createPublicClient({
      chain: sepolia,
      transport: http(RPC_URL),
    ...
    // Token 余额
    console.log("Account Address:", account.address); // 调试：打印地址
    ...
    const tokenBalance = await publicClient.readContract({
      ...
      args: [account.address], // 直接使用 account.address
    });
    ...
    console.error("Error fetching balances:", error); // 打印完整的错误信息
  }
};
